...
import * as DateUtils from '@/contexts/useDate';
...
export const convertRiskLevelToRiskTitle = (level: number) => {
  const RISK_PROFILE = {
    1: `t_very_conservative`,
    2: `t_conservative`,
    3: `t_moderate`,
    4: `t_aggresive`,
    5: `t_very_aggresive`,
  ...
  return RISK_PROFILE[level as keyof typeof RISK_PROFILE];
};

export const print = async (iterable: HTMLElement[]) => {
  ...
  for await (const [_, item] of iterable.entries()) {
    ...
    const leftAndRightMargin = 2 * margin;
    const imageWidth = pdf.internal.pageSize.getWidth() - leftAndRightMargin;
    const imageHeight =
      (imgProperties.height * imageWidth) / imgProperties.width;
    ...
    while (remainingHeight >= 0) {
      ...
      position = position + remainingHeight - imageHeight;
      ...
      remainingHeight = remainingHeight - pdf.internal.pageSize.getHeight();
    }

    position += imageHeight + 5;
  }
...
export const calculateCompound = (
  amount: number = 0,
  rate: number = 0, // percentage
  initialLabel: number = 0,
  duration: number = 0,
) => {
  ...
  Array.from({ length: duration }).forEach((_, index) => {
    labels.push(initialLabel + index + 1);
    values.push((values.at(-1) || 0) * (1 + growthRate));
  });

  return { labels, values };
};

export const convertPeriodToDate = (
  period: `1W` | `1M` | `3M` | `6M` | `12M` | `YTD`,
  endDate: string,
) => {
  let startDate = ``;
  if (period === `1W`) {
    startDate = DateUtils.format(DateUtils.substract(endDate, 7), `YYYY-MM-DD`);
  }
  if (period === `1M`) {
    startDate = DateUtils.format(
      DateUtils.substract(endDate, 1, `M`),
      `YYYY-MM-DD`,
    );
  }
  if (period === `3M`) {
    startDate = DateUtils.format(
      DateUtils.substract(endDate, 3, `M`),
      `YYYY-MM-DD`,
    );
  }
  if (period === `6M`) {
    startDate = DateUtils.format(
      DateUtils.substract(endDate, 6, `M`),
      `YYYY-MM-DD`,
    );
  }
  if (period === `12M`) {
    startDate = DateUtils.format(
      DateUtils.substract(endDate, 12, `M`),
      `YYYY-MM-DD`,
    );
  }
  if (period === `YTD`) {
    startDate = DateUtils.format(endDate, `YYYY`) + `-01-01`;
  }
  return startDate;
};
